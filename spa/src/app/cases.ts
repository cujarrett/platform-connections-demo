...
const WORKSPACES_SHA = "741adc110010af467871129fc518b5307b378757"
...
const COMPOSITION_SHA = "3d667ecc76e510bfa3d17da9309e4d300bf802bb"
...
const CONNECTIONS_DOC: Source = {
  file: "platform/docs/connections.md",
  note: "how the mesh enforces every call on this page",
  url: `${HOMELAB}/platform/docs/connections.md`,
}
...
<p><b>Neither team wrote a service mesh policy.</b> One declared what it calls. Platform engineering is the difference between understanding a mesh and shipping without needing to.</p>`,
  ...
  - namespace: platform-connections-demo
    app: upstream-api`,
        sources: [workspace("authorized-api.yaml", "the caller's way out", 17, 19)],
        actor: "caller",
      },
    ],
    rendered: [
...
spec:
  ...
  egress:
    - hosts:
        ...
        sources: [composition("the Sidecar egress list", 1048, 1095)],
        actor: "caller",
      },
      {
...
spec:
  ...
  portLevelMtls:
    ...
    "9090":
      mode: PERMISSIVE`,
        sources: [composition("the PeerAuthentication template", 1024, 1046)],
        actor: "callee",
      },
    ],
  },
...
# upstream-api is absent, so it is unreachable`,
        sources: [
          workspace("unauthorized-api.yaml", "the caller, same image, nothing declared", 1, 13),
        ],
        actor: "caller",
      },
    ],
...
# upstream-api is on no list, so there is nowhere to send it`,
        sources: [composition("REGISTRY_ONLY and the Sidecar egress list", 1048, 1095)],
        actor: "caller",
      },
    ],
  },
...
# declared, so this one is reachable`,
        sources: [workspace("authorized-api.yaml", "the declared host", 17, 18)],
        actor: "caller",
      },
    ],
    rendered: [
  ...
  ports:
    - number: 443
      ...
      protocol: TLS      # the app's own TLS passes straight through`,
        sources: [composition("the ServiceEntry per declared host", 1096, 1150)],
        actor: "caller",
      },
      {
...
# no consumes entry for the endpoint, the ref is the declaration`,
        sources: [
          workspace("authorized-api.yaml", "the ref, which is the whole declaration", 15, 16),
...
spec:
  ...
  ports:
    - number: 443
      name: tls
      protocol: TLS`,
        sources: [composition("the ServiceEntry per derived endpoint", 1096, 1150)],
        actor: "caller",
      },
    ],
  },
...
# which API to ask a token for`,
        sources: [
          workspace("authorized-api.yaml", "the line the Entra scope is built from", 17, 19),
        ],
        actor: "caller",
      },
      {
  ...
  - name: Data.Read
    ...
    allowedCallers:
      - namespace: platform-connections-demo
        app: authorized-api`,
        sources: [
          workspace("upstream-api.yaml", "the callee declares the role, and who holds it", 13, 17),
        ],
        actor: "callee",
      },
    ],
  ...
  forProvider:
    ...
    api:
      ...
      requestedAccessTokenVersion: 2`,
        sources: [composition("the Application template", 732, 771)],
        actor: "caller",
      },
      {
  ...
  forProvider:
    ...
    audiences:
      - api://AzureADTokenExchange`,
        sources: [composition("the FederatedIdentityCredential template", 789, 806)],
        actor: "caller",
      },
      {
...
spec:
  forProvider:
    ...
    allowedMemberTypes: ["Application"]`,
        sources: [composition("the AppRole template", 819, 838)],
        actor: "callee",
      },
      {
  ...
  forProvider:
    ...
    resourceObjectIdRef:
      name: platform-connections-demo-upstream-api-entra`,
        sources: [composition("the RoleAssignment template", 841, 860)],
        actor: "callee",
      },
    ],
  },
...
# unchanged. the caller asks for nothing route-specific`,
        sources: [workspace("authorized-api.yaml", "the same line as the card above", 17, 19)],
        actor: "caller",
      },
      {
        code: `provides:
  ...
  - name: Data.Admin
    auth: workload
    allowedCallers: []`,
        sources: [workspace("upstream-api.yaml", "two interfaces, one grant", 13, 22)],
        actor: "callee",
      },
    ],
    rendered: [
...
spec:
  forProvider:
    value: Data.Admin
    allowedMemberTypes: ["Application"]`,
        sources: [composition("one AppRole per declared role", 819, 838)],
        actor: "callee",
      },
      {
...
spec:
  forProvider:
    principalObjectIdRef:
      name: platform-connections-demo-authorized-api-entra`,
        sources: [composition("one grant per allowed caller, and no more", 841, 860)],
        actor: "callee",
      },
    ],
  },
  ...
  <text x="830" y="362" class="d-d" fill="#a5b4fc">upstream-api</text>
    ...
    docs: [CONNECTIONS_DOC, APP_CONFIG_DOC, NOTHING_NOVEL],
  },
]
